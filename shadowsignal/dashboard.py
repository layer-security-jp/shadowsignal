...
form.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    results.innerHTML = body.items.map((item, index) => {
      ...
      const eventCount = item.api_request.events?.length || 0;
      return `<article class="result-card"><span class="badge">SESSION ${index+1}</span><h2>${esc(title)}</h2>
        <div class="facts"><div class="fact"><span>通信形状</span>${esc(api.shape_verdict || (body.dry_run ? "未送信" : null))}</div><div class="fact"><span>信頼度</span>${esc(api.confidence_bucket)}</div><div class="fact"><span>イベント数</span>${eventCount}</div></div>
